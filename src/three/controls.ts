...
export var Controls = function (object, domElement) {
  ...
  this.controlsActive = function () { return (state === STATE.NONE); };
  this.setPan = function (vec3) { pan = vec3; };
  ...
  this.panXY = function (x, y) { scope.pan(new THREE.Vector2(x, y)); };
  ...
  function onKeyDown(event) {
    ...
    switch (event.keyCode) {
      case scope.keys.UP: scope.pan(new THREE.Vector2(0, scope.keyPanSpeed)); break;
      case scope.keys.BOTTOM: scope.pan(new THREE.Vector2(0, -scope.keyPanSpeed)); break;
      case scope.keys.LEFT: scope.pan(new THREE.Vector2(scope.keyPanSpeed, 0)); break;
      case scope.keys.RIGHT: scope.pan(new THREE.Vector2(-scope.keyPanSpeed, 0)); break;
    }
  }
  ...
  function touchmove(event) {
    ...
    switch (event.touches.length) {
      case 1:
        if (scope.noRotate === true || state !== STATE.TOUCH_ROTATE) return;
        ...
        break;
      case 2:
        if (scope.noZoom === true || state !== STATE.TOUCH_DOLLY) return;
        var dx = event.touches[0].pageX - event.touches[1].pageX;
        var dy = event.touches[0].pageY - event.touches[1].pageY;
        dollyEnd.set(0, Math.sqrt(dx * dx + dy * dy));
        dollyDelta.subVectors(dollyEnd, dollyStart);
        if (dollyDelta.y > 0) scope.dollyOut(); else scope.dollyIn();
        dollyStart.copy(dollyEnd);
        break;
      case 3:
        if (scope.noPan === true || state !== STATE.TOUCH_PAN) return;
